// Boring utils folder that I never look at.
...
export async function extractArchiveTo(packageBuffer, target, {virtualPath = 0} = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        let extractor = tarFs.extract(target, {
            map
        });
        gunzipper.pipe(extractor);
...
export async function extractNpmArchiveTo(packageBuffer, target) {
    ...
    return await extractArchiveTo(packageBuffer, target, {
            virtualPath: 1
        });
...
export async function trackProgress(cb) {
    ...
    let pace = new Progress(`:bar :current/:total (:elapseds)`, {
        width: 80,
        total: 1
    });
